...
In this vignette we explain how to expand the **omopgenerics** ecosystem by defining more sources.

## The source object

First we need to define a function to create our source object. The source object must be an object (usually a list) that contains several attributes that will be used by the methods. Finally, we have to assign a class to our source and validate it with `omopgenerics::newCdmSource()`. The function has an argument to assign a `sourceType` that must be a character vector identifying the name of the source. This is what will be retrieved by the `omopgenerics::sourceType()` function, and it will be useful for identifying how the source of the cdm_reference has been created.
...
If the first function that we create is `myCustomSource()`, validation with `omopgenerics::newCdmSource()` will fail because the required *methods* are checked to confirm that they are defined and work properly.
...
- `listSourceTables` **required** To list the data present in your source.
...
**Output**: The output of `insertTable()` must be a `cdm_table`, so your function must validate it at the end with `omopgenerics::newCdmTable()`.
...
**Output**: The output of `readSourceTable()` must be a `cdm_table`, so your function must validate it at the end with `omopgenerics::newCdmTable()`.
...
**Output**: The output is ignored; we recommend returning the source invisibly.
...
- `cdm` A cdm reference from a different source. We recommend collecting each table before inserting it.
...
This function works slightly differently from the rest; the input will be a query instead of the source object.
...
**Output**: The output of `compute()` must be a reference to your table in your source data. It will be converted later to a cdm_table, but you do not have to worry about that.
...
Finally, every `<cdm_source>` class object also needs a function to create a *<cdm_reference>*. To do that, you just have to read all the tables that you want to include in your cdm object. **tables** must be a list of *<cdm_table>* objects with the same source.
